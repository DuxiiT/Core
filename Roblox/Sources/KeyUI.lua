...
if getgenv().CoreLoaded and hui:FindFirstChild("CoreKeySystem") then return getgenv().Core end
if getgenv().CoreLoaded and hui:FindFirstChild("CoreKeylessSystem") then return getgenv().Core end
getgenv().CoreLoaded = true
getgenv().CoreClosed = false
...
Core.Appearance = {
    ...
    Icon = "rbxassetid://116265951007418",
...
local function getFileName()
    return FolderName .. "/" .. Core.Storage.FileName .. ".txt"
end

local function saveKey(key)
    if not fileSystemSupported or not Core.Storage.Remember then return false end
...
local function getLogoIcon()
    if Core.Appearance.Icon == DefaultLogoAsset then return getIcon("logo") end
    return Core.Appearance.Icon
end

local function shouldDownloadLogo()
    return Core.Appearance.Icon == DefaultLogoAsset
end

local function getShopIcon()
    if Core.Shop.Icon == "" then return getLogoIcon() end
    return Core.Shop.Icon
end

local function isShopEnabled()
    return Core.Shop.Enabled
end
...
local function enableBlur()
    if not Core.Options.Blur then return end
    local existing = Lighting:FindFirstChild("CoreKeySystemBlur")
    if existing then existing:Destroy() end
    Internal.BlurEffect = Instance.new("BlurEffect")
    Internal.BlurEffect.Name = "CoreKeySystemBlur"
...
local function fullCleanup()
    getgenv().CoreLoaded = false
    getgenv().CoreClosed = true
    disableBlur()
    local gui1 = hui:FindFirstChild("CoreKeySystem")
    local gui2 = hui:FindFirstChild("CoreKeylessSystem")
    local gui3 = hui:FindFirstChild("CoreLoadingScreen")
...
local function setupDragging(header, main)
    if not Core.Options.Draggable then return end
...
local function CreateDoorOverlay(parentFrame, width, height)
    ...
    leftDoor.BackgroundColor3 = Core.Theme.Header
    ...
    rightDoor.BackgroundColor3 = Core.Theme.Header
    ...
    logoImage.ImageColor3 = Core.Theme.Text
...
local function ShowLoadingScreen(onComplete)
    ...
    local oldGui = hui:FindFirstChild("CoreLoadingScreen")
    if oldGui then oldGui:Destroy() end
    local oldBlur = Lighting:FindFirstChild("CoreLoadingBlur")
    if oldBlur then oldBlur:Destroy() end

    local blurEffect = Instance.new("BlurEffect")
    blurEffect.Name = "CoreLoadingBlur"
    blurEffect.Size = 0
    blurEffect.Parent = Lighting

    local gui = Instance.new("ScreenGui")
    gui.Name = "CoreLoadingScreen"
    ...
    for i = 1, 4 do
        ...
        line.BackgroundColor3 = Core.Theme.Text
    ...
    shipBody.BackgroundColor3 = Core.Theme.Text
    ...
    shipPoint.BackgroundColor3 = Core.Theme.Text
    ...
    for i, config in ipairs(trailConfigs) do
        ...
        trail.BackgroundColor3 = Core.Theme.Text
    ...
    for i, name in ipairs(phaseNames) do
        ...
        indicator.Text = "○"
        indicator.TextColor3 = Core.Theme.Pending
        ...
        label.Text = name
        label.TextColor3 = Core.Theme.Pending
    ...
    local function setPhase(num)
        ...
        for i = 1, 5 do
            ...
            if i < num then
                p.indicator.Text = "●"
                TweenService:Create(p.indicator, TweenInfo.new(0.2), {TextColor3 = Core.Theme.Success, TextTransparency = 0}):Play()
                TweenService:Create(p.label, TweenInfo.new(0.2), {TextColor3 = Core.Theme.Success}):Play()
            elseif i == num then
                p.indicator.Text = "●"
                p.indicator.TextTransparency = 0
                TweenService:Create(p.indicator, TweenInfo.new(0.2), {TextColor3 = Core.Theme.Accent}):Play()
                TweenService:Create(p.label, TweenInfo.new(0.2), {TextColor3 = Core.Theme.Text}):Play()
                currentPhase = num
                ...
                p.indicator.Text = "○"
                p.indicator.TextColor3 = Core.Theme.Pending
                p.label.TextColor3 = Core.Theme.Pending
            end
        end
    end
...
function Core:Notify(title, message, duration, iconType)
    ...
    frame.BackgroundColor3 = Core.Theme.Header
    frame.BorderSizePixel = 0
    frame.Parent = notifGui
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 4)

    local stroke = Instance.new("UIStroke", frame)
    stroke.Color = Core.Theme.Accent
    stroke.Thickness = 1
    ...
    progressBar.BackgroundColor3 = Core.Theme.Accent
    ...
    local iconMap = {
        success = {"check", Core.Theme.Success}, error = {"alert", Core.Theme.Error},
        warning = {"alert", Core.Theme.Warning}, shield = {"shield", Core.Theme.Accent},
        info = {"shield", Core.Theme.Accent}, key = {"key", Core.Theme.Accent},
        copy = {"copy", Core.Theme.Success}, discord = {"discord", Core.Theme.Discord},
        close = {"close", Core.Theme.Error}
    }
    ...
    if iconMap[iconType] then
        ...
        icon.ImageColor3 = Core.Theme.Text
    end
    ...
    titleLabel.TextColor3 = Core.Theme.Text
    ...
    messageLabel.TextColor3 = Core.Theme.TextDim
...
local function CreateChangelogPanel(parent, windowWidth, panelHeight, panelWidth, mainFrame, gap)
    ...
    panel.BackgroundColor3 = Core.Theme.Background
    ...
    panelStroke.Color = Core.Theme.Accent
    panelStroke.Thickness = 2
    panelStroke.Transparency = 1

    local panelHeader = Instance.new("Frame")
    panelHeader.Size = UDim2.new(1, 0, 0, 50)
    panelHeader.BackgroundColor3 = Core.Theme.Header
    ...
    panelHeaderFix.BackgroundColor3 = Core.Theme.Header
    ...
    panelHeaderLine.BackgroundColor3 = Core.Theme.Accent
    ...
    panelHeaderIcon.ImageColor3 = Core.Theme.Accent
    ...
    panelTitle.TextColor3 = Core.Theme.Text
    ...
    panelClose.ImageColor3 = Core.Theme.TextDim
    panelClose.ScaleType = Enum.ScaleType.Fit
    panelClose.Parent = panelHeader
    panelClose.MouseEnter:Connect(function() TweenService:Create(panelClose, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.Error}):Play() end)
    panelClose.MouseLeave:Connect(function() TweenService:Create(panelClose, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.TextDim}):Play() end)
    ...
    scrollFrame.ScrollBarImageColor3 = Core.Theme.Accent
    ...
    for i, update in ipairs(Core.Changelog) do
        ...
        versionLabel.TextColor3 = Core.Theme.Accent
        ...
        for j, change in ipairs(update.Changes) do
            ...
            changeLabel.TextColor3 = Core.Theme.TextDim
        ...
        if i < #Core.Changelog then
            ...
            div.BackgroundColor3 = Core.Theme.Divider
...
local function CreateUserInfoPanel(parent, windowWidth, panelHeight, panelWidth, mainFrame, gap, startOpen)
    ...
    avatarGlow.BackgroundColor3 = Core.Theme.Accent
    ...
    avatarGlowStroke.Color = Core.Theme.Accent
    ...
    avatarContainer.BackgroundColor3 = Core.Theme.Input
    ...
    welcomeLabel.TextColor3 = Core.Theme.Text
    ...
    divider1.BackgroundColor3 = Core.Theme.Divider
    ...
    executorTitle.TextColor3 = Core.Theme.TextDim
    ...
    executorValue.TextColor3 = Core.Theme.Accent
    ...
    deviceTitle.TextColor3 = Core.Theme.TextDim
    ...
    deviceValue.TextColor3 = Core.Theme.Accent
    ...
    divider2.BackgroundColor3 = Core.Theme.Divider
    ...
    hwidTitle.Text = "HWID"
    hwidTitle.TextColor3 = Core.Theme.TextDim
    ...
    hwidValue.TextColor3 = Core.Theme.TextDim
    ...
    copyBtn.ImageColor3 = Core.Theme.TextDim
    copyBtn.ScaleType = Enum.ScaleType.Fit
    copyBtn.Parent = hwidContainer
    copyBtn.MouseEnter:Connect(function() TweenService:Create(copyBtn, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.Accent}):Play() end)
    copyBtn.MouseLeave:Connect(function() TweenService:Create(copyBtn, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.TextDim}):Play() end)
    copyBtn.MouseButton1Click:Connect(function()
        pcall(function() setclipboard(fullHWID) end)
        TweenService:Create(copyBtn, TweenInfo.new(0.1), {ImageColor3 = Core.Theme.Success}):Play()
        task.delay(0.3, function() TweenService:Create(copyBtn, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.TextDim}):Play() end)
        Core:Notify("Copied", "HWID copied to clipboard", 2, "copy")
    end)
    ...
    divider3.BackgroundColor3 = Core.Theme.Divider
    ...
    clockIcon.ImageColor3 = Core.Theme.Accent
    ...
    clockTimeLabel.TextColor3 = Core.Theme.Accent
    ...
    clockDateLabel.TextColor3 = Core.Theme.TextDim
...
local function handleKeylessSkip()
    ...
    getgenv().CoreLoaded = false
    Core:Notify("Access Granted", "Keyless access approved!", 3, "success")
    task.wait(0.3)
    if Core.Callbacks.OnSuccess then Core.Callbacks.OnSuccess() end
end
...
local function BuildCenteredUI(windowWidth, windowHeight, panelHeight, userPanelWidth, changelogPanelWidth, gap, buildContent)
    ...
    mainFrame.BackgroundColor3 = Core.Theme.Background
    mainFrame.BorderSizePixel = 0
    mainFrame.Parent = container
    Instance.new("UICorner", mainFrame).CornerRadius = UDim.new(0, 4)

    local mainStroke = Instance.new("UIStroke", mainFrame)
    mainStroke.Color = Core.Theme.Accent
...
local function BuildKeylessUI()
    local oldGui = hui:FindFirstChild("CoreKeylessSystem")
    if oldGui then oldGui:Destroy() end
    local oldGui2 = hui:FindFirstChild("CoreKeySystem")
    ...
    header.BackgroundColor3 = Core.Theme.Header
    ...
    headerFix.BackgroundColor3 = Core.Theme.Header
    ...
    headerLine.BackgroundColor3 = Core.Theme.Accent
    ...
    logo.ImageColor3 = Core.Theme.Text
    ...
    title.Text = Core.Appearance.Title
    title.TextColor3 = Core.Theme.Text
    ...
    closeBtn.ImageColor3 = Core.Theme.TextDim
    closeBtn.ScaleType = Enum.ScaleType.Fit
    closeBtn.Parent = header
    closeBtn.MouseEnter:Connect(function() TweenService:Create(closeBtn, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.Error}):Play() end)
    closeBtn.MouseLeave:Connect(function() TweenService:Create(closeBtn, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.TextDim}):Play() end)
    ...
    successBox.BackgroundColor3 = Core.Theme.Success
    ...
    successStroke.Color = Core.Theme.Success
    ...
    checkIcon.ImageColor3 = Core.Theme.Success
    ...
    successText.TextColor3 = Core.Theme.Success
    ...
    keylessText.TextColor3 = Core.Theme.TextDim
    ...
    divider.BackgroundColor3 = Core.Theme.Divider
    ...
    launchBtn.BackgroundColor3 = Core.Theme.Accent
    ...
    launchStroke.Color = Core.Theme.AccentHover
    ...
    launchIcon.ImageColor3 = Core.Theme.Text
    ...
    launchLabel.TextColor3 = Core.Theme.Text
    ...
    launchBtn.MouseEnter:Connect(function() TweenService:Create(launchBtn, TweenInfo.new(0.15), {BackgroundColor3 = Core.Theme.AccentHover}):Play() end)
    launchBtn.MouseLeave:Connect(function() TweenService:Create(launchBtn, TweenInfo.new(0.15), {BackgroundColor3 = Core.Theme.Accent}):Play() end)
    ...
    userBtn.BackgroundColor3 = Core.Theme.Background
    ...
    userIcon.ImageColor3 = Core.Theme.TextDim
    userIcon.ScaleType = Enum.ScaleType.Fit
    userIcon.Parent = userBtn
    userBtn.MouseEnter:Connect(function() TweenService:Create(userIcon, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.Accent}):Play() end)
    userBtn.MouseLeave:Connect(function() TweenService:Create(userIcon, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.TextDim}):Play() end)
    ...
    discordBtn.BackgroundColor3 = Core.Theme.Background
    ...
    discordIcon.ImageColor3 = Core.Theme.Discord
    discordIcon.ScaleType = Enum.ScaleType.Fit
    discordIcon.Parent = discordBtn
    discordBtn.MouseEnter:Connect(function() TweenService:Create(discordIcon, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.DiscordHover}):Play() end)
    discordBtn.MouseLeave:Connect(function() TweenService:Create(discordIcon, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.Discord}):Play() end)
    ...
    changelogBtn.BackgroundColor3 = Core.Theme.Background
    ...
    changelogIcon.ImageColor3 = Core.Theme.TextDim
    changelogIcon.ScaleType = Enum.ScaleType.Fit
    changelogIcon.Parent = changelogBtn
    changelogBtn.MouseEnter:Connect(function() TweenService:Create(changelogIcon, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.Text}):Play() end)
    changelogBtn.MouseLeave:Connect(function() TweenService:Create(changelogIcon, TweenInfo.new(0.15), {ImageColor3 = Core.Theme.TextDim}):Play() end)

    if #Core.Changelog == 0 then
    ...
    closeBtn.MouseButton1Click:Connect(function()
        Core:Notify("Goodbye", "See you next time!", 2, "close")
        ...
        if Core.Callbacks.OnClose then Core.Callbacks.OnClose() end
    end)

    launchBtn.MouseButton1Click:Connect(function()
        Core:Notify("Launching", "Script loaded successfully!", 2, "success")
        getgenv().SCRIPT_KEY = "KEYLESS"
        getgenv().CoreLoaded = false
        closeDoorsThenExit(function()
            disableBlur()
            ...
            TweenService:Create(mainStroke, TweenInfo.new(0.3), {Transparency = 1}):Play()
            task.wait(0.4) gui:Destroy()
            if not Internal.IsJunkieMode and Core.Callbacks.OnSuccess then Core.Callbacks.OnSuccess() end
        end)
    end)

    discordBtn.MouseButton1Click:Connect(function()
        if Core.Links.Discord ~= "" then
            Core:Notify("Discord", "Invite link copied!", 2, "discord")
            pcall(function() setclipboard(Core.Links.Discord) end)
        end
    end)
    ...
    doors.open(function()
        ...
        if #Core.Changelog > 0 then task.wait(0.3) ui.toggleCL(changelogIcon) end
    end)
end

local function BuildKeyUI()
    local oldGui = hui:FindFirstChild("CoreKeySystem")
    if oldGui then oldGui:Destroy() end
    local oldGui2 = hui:FindFirstChild("CoreKeylessSystem")
    if oldGui2 then oldGui2:Destroy() end
    ...
    local logo = Instance.new("ImageLabel")
    logo.Size = Core.Appearance.IconSize
    logo.Position = UDim2.new(0, padding, 0.5, 0)
    logo.AnchorPoint = Vector2.new(0, 0.5)
    logo.BackgroundTransparency = 1
    logo.Image = getLogoIcon()
    ...
    titleLabel.Position = UDim2.new(0, padding + Core.Appearance.IconSize.X.Offset + 10, 0, 0)
    titleLabel.BackgroundTransparency = 1
    titleLabel.Text = Core.Appearance.Title
    titleLabel.TextColor3 = Core.Theme.Text
    ...
    statusFrame.BackgroundColor3 = Core.Theme.Input
    ...
    statusStroke.Color = Core.Theme.Accent
    ...
    statusIcon.ImageColor3 = Core.Theme.StatusIdle
    ...
    statusLabel.Text = Core.Appearance.Subtitle
    statusLabel.TextColor3 = Core.Theme.StatusIdle
    ...
    inputFrame.BackgroundColor3 = Core.Theme.Input
    ...
    inputStroke.Color = Core.Theme.Accent
    ...
    textBox.Text = ""
    textBox.TextColor3 = Core.Theme.Text
    textBox.PlaceholderText = "Enter your key..."
    textBox.PlaceholderColor3 = Core.Theme.TextDim
    ...
    dividerLine.BackgroundColor3 = Core.Theme.Divider
    ...
    local function createButton(text, iconKey, isPrimary, yPos)
        ...
        btn.BackgroundColor3 = isPrimary and Core.Theme.Accent or Core.Theme.Input
        ...
        btnStroke.Color = isPrimary and Core.Theme.AccentHover or Core.Theme.Accent
        ...
        iconImg.ImageColor3 = Core.Theme.Text
        ...
        label.BackgroundTransparency = 1
        label.Text = text
        label.TextColor3 = Core.Theme.Text
        ...
        local hoverColor = isPrimary and Core.Theme.AccentHover or Core.Theme.Accent
    ...
    if showShop then
        ...
        shopDivider.BackgroundColor3 = Core.Theme.Accent
        ...
        shopFrame.BackgroundColor3 = Core.Theme.Header
        ...
        shopTopFix.BackgroundColor3 = Core.Theme.Header
        ...
        shopIconWrapper.BackgroundColor3 = Core.Theme.Accent
        ...
        shopIconStroke.Color = Core.Theme.Accent
        ...
        shopIconImg.ImageColor3 = Core.Theme.Text
        ...
        shopTitle.Text = Core.Shop.Title
        shopTitle.TextColor3 = Core.Theme.Text
        ...
        shopSubtitle.Text = Core.Shop.Subtitle
        shopSubtitle.TextColor3 = Core.Theme.TextDim
        ...
        buyBtn.BackgroundColor3 = Core.Theme.Accent
        ...
        buyBtnStroke.Color = Core.Theme.AccentHover
        ...
        buyIcon.ImageColor3 = Core.Theme.Text
        ...
        buyLabel.Text = Core.Shop.ButtonText
        buyLabel.TextColor3 = Core.Theme.Text
        buyLabel.TextSize = mobile and 11 or 12
        buyLabel.Font = Enum.Font.ArimoBold
        buyLabel.LayoutOrder = 2
        buyLabel.Parent = buyContent

        buyBtn.MouseEnter:Connect(function() TweenService:Create(buyBtn, TweenInfo.new(0.15), {BackgroundColor3 = Core.Theme.AccentHover}):Play() end)
        buyBtn.MouseLeave:Connect(function() TweenService:Create(buyBtn, TweenInfo.new(0.15), {BackgroundColor3 = Core.Theme.Accent}):Play() end)
        buyBtn.MouseButton1Click:Connect(function()
            if Core.Shop.Link ~= "" then
                pcall(function() setclipboard(Core.Shop.Link) end)
                Core:Notify("Shop", "Shop link copied to clipboard!", 2, "copy")
            end
        end)
    end
    ...
    local function setStatus(state, customText)
        ...
        local color, icon, text = Core.Theme.StatusIdle, getIcon("lock"), customText or "No key detected"
        if state == "verifying" then
            color, icon, text = Core.Theme.Accent, getIcon("loading"), "Verifying key"
        ...
        elseif state == "success" then color, icon, text = Core.Theme.Success, getIcon("check"), customText or "Access Granted"
        elseif state == "error" then color, icon, text = Core.Theme.Error, getIcon("alert"), customText or "Invalid Key" end
    ...
    local function handleRedeem()
        ...
        if key == "" then Core:Notify("Error", "Please enter your key", 3, "warning") return end
        ...
        if valid then
            saveKey(key) getgenv().SCRIPT_KEY = key getgenv().CoreLoaded = false
            setStatus("success") Core:Notify("Success", "Key validated successfully!", 2, "success") task.wait(1)
            closeDoorsThenExit(function()
                disableBlur()
                TweenService:Create(container, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {Position = UDim2.new(0.5, 0, -0.5, 0)}):Play()
                TweenService:Create(mainFrame, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            ...
            setStatus("error", errorMsg) Core:Notify("Invalid", errorMsg, 4, "error")
            if Core.Callbacks.OnFail then Core.Callbacks.OnFail(errorMsg) end
        end
    end

    redeemBtn.MouseButton1Click:Connect(handleRedeem)
    acquireBtn.MouseButton1Click:Connect(function()
        if Core.Links.GetKey ~= "" then Core:Notify("Copied", "Key link copied!", 3, "copy") pcall(function() setclipboard(Core.Links.GetKey) end)
        else Core:Notify("Error", "No key link set", 3, "warning") end
    end)
    discordBtn.MouseButton1Click:Connect(function()
        if Core.Links.Discord ~= "" then Core:Notify("Discord", "Invite link copied!", 2, "discord") pcall(function() setclipboard(Core.Links.Discord) end) end
    end)
...
function Core:Launch()
    Internal.IsJunkieMode = false
    Internal.ValidateFunction = Core.Callbacks.OnVerify
    local existingKey = getgenv().SCRIPT_KEY
    if existingKey and existingKey ~= "" then
        if existingKey == "KEYLESS" then
            Core:Notify("Executed", "Script loaded successfully!", 2, "success")
            if Core.Callbacks.OnSuccess then Core.Callbacks.OnSuccess() end return
    ...
    EnsureIconsReady(function()
        if Core.Options.Keyless == true then
            if Core.Options.KeylessUI == false then handleKeylessSkip() return end
            BuildKeylessUI()
            while not getgenv().SCRIPT_KEY do task.wait(0.1) end
            return
        end
        if Core.Storage.AutoLoad and Internal.ValidateFunction then
            local savedKey = loadKey()
            if savedKey and savedKey ~= "" then
                Core:Notify("Checking", "Validating saved key...", 2, "shield") task.wait(0.5)
                if validateKey(savedKey, Internal.ValidateFunction) then
                    getgenv().SCRIPT_KEY = savedKey
                    Core:Notify("Welcome Back", "Key validated!", 2, "success")
                    if Core.Callbacks.OnSuccess then Core.Callbacks.OnSuccess() end return
                else clearKey() Core:Notify("Expired", "Saved key is no longer valid", 3, "warning") task.wait(1) end
            end
        end
        BuildKeyUI()
        while not getgenv().SCRIPT_KEY do task.wait(0.1) end
...
function Core:LaunchJunkie(config)
    ...
    EnsureIconsReady(function()
        ...
        if not success or not Junkie then Core:Notify("Error", "Failed to load Junkie SDK", 5, "error") return end
        Junkie.service = config.Service
        Junkie.identifier = config.Identifier
        Junkie.provider = config.Provider
        Internal.Junkie = Junkie
        if Core.Links.GetKey == "" then pcall(function() Core.Links.GetKey = Junkie.get_key_link() end) end
        Internal.ValidateFunction = function(key) return Junkie.check_key(key) end
        if Core.Options.Keyless == nil then
        ...
        elseif Core.Options.Keyless == true then
            if Core.Options.KeylessUI == false then handleKeylessSkip() return end
            BuildKeylessUI()
            while not getgenv().SCRIPT_KEY do task.wait(0.1) end
            return
        end
        if Core.Storage.AutoLoad then
            local savedKey = loadKey()
            if savedKey and savedKey ~= "" then
                Core:Notify("Checking", "Validating saved key...", 2, "shield") task.wait(0.5)
...
function Core:GetSavedKey() return loadKey() end
function Core:ClearSavedKey() return clearKey() end
